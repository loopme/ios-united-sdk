...
//    [self.vastEventTracker trackEvent:LoopMeVASTEventTypeLinearCreativeView];
    ...
    [[LoopMeLifecycleManager shared] updateSessionDepthForKey:self.delegate.appKey];
...
- (void)setAdConfiguration:(LoopMeAdConfiguration *)configuration {
    if (configuration) {
        ...
        if (!configuration.vastProperties.isVpaid) {
            NSError *error;
            self.omidSession = [self.omidWrapper sessionForNativeVideoWithOmidJS:[[LoopMeSDK shared] getJSStringFromResources:@"omsdk.js"] resources:configuration.vastProperties.adVerifications  error:&error];
