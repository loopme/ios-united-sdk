...
@interface LoopMeDiskLRUCacheFile : NSObject
...
@implementation LoopMeDiskLRUCacheFile


@end

@interface LoopMeVASTDiskURLCache ()
...
@implementation LoopMeVASTDiskURLCache

+ (LoopMeVASTDiskURLCache *)sharedDiskCache {
    static dispatch_once_t once;
    static LoopMeVASTDiskURLCache *sharedDiskCache;
    dispatch_once(&once, ^{
...
- (id)init {
    self = [super init];
    if (self != nil) {
        _diskQueue = dispatch_queue_create("com.loopme.diskCacheQueue", DISPATCH_QUEUE_SERIAL);

        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
        if (paths.count > 0) {
            _diskCachePath = [[[paths objectAtIndex:0] stringByAppendingPathComponent:@"com.loopme.diskCache"] copy];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        dispatch_async(self.diskQueue, ^{
            @autoreleasepool {
                ...
                for (LoopMeDiskLRUCacheFile *file in expiredFiles) {
                    NSLog(@"Trying to remove %@ from cache due to expiration", file.filePath);
                ...
                while ([self sizeOfCacheFilesInArray:cacheFilesSortedByModDate] >= kCacheSoftMaxSize && cacheFilesSortedByModDate.count > 0) {
                    NSString *oldestFilePath = ((LoopMeDiskLRUCacheFile *)[cacheFilesSortedByModDate objectAtIndex:0]).filePath;

                    NSLog(@"Trying to remove %@ from cache due to size", oldestFilePath);
...
- (NSArray *)expiredCachedFilesInArray:(NSArray *)cachedFiles {
    ...
    for (LoopMeDiskLRUCacheFile *file in cachedFiles) {
...
- (NSMutableArray *)cacheFilesSortedByModDate {
    ...
    for (NSString *fileName in sortedFiles) {
        ...
        LoopMeDiskLRUCacheFile *cacheFile = [[LoopMeDiskLRUCacheFile alloc] init];
...
- (uint64_t)sizeOfCacheFilesInArray:(NSArray *)files {
    ...
    for (LoopMeDiskLRUCacheFile *file in files) {
        currentSize += file.fileSize;
    }

    LoopMeLogDebug(@"Current cache size %qu bytes", currentSize);

    return currentSize;
}

- (NSString *)cacheFilePathForKey:(NSString *)key {
    NSString *hashedKey = [NSString lm_MD5:key];
