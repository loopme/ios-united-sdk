...
final class ResourceLoaderDelegate: NSObject, AVAssetResourceLoaderDelegate, URLSessionDelegate, URLSessionDataDelegate, URLSessionTaskDelegate {
    ...
    private static let queue = DispatchQueue(label: "com.gcd.CachingPlayerItemQueue", qos: .userInitiated, attributes: .concurrent)
    private static var pendingRequests: Set<AVAssetResourceLoadingRequest> {
        get { queue.sync { return pendingRequestsValue } }
        set { queue.async(flags: .barrier) { self.pendingRequestsValue = newValue } }
    }
    private static var pendingRequestsValue = Set<AVAssetResourceLoadingRequest>()
    ...
    func resourceLoader(_ resourceLoader: AVAssetResourceLoader, shouldWaitForLoadingOfRequestedResource loadingRequest: AVAssetResourceLoadingRequest) -> Bool {
        ...
        if ResourceLoaderDelegate.pendingRequests.contains(where: {$0.request.url?.path == url.path}) {
            return true
        }
        ...
        ResourceLoaderDelegate.pendingRequests.insert(loadingRequest)
        processPendingRequests()
        return true
    }

    func resourceLoader(_ resourceLoader: AVAssetResourceLoader, didCancel loadingRequest: AVAssetResourceLoadingRequest) {
        ResourceLoaderDelegate.pendingRequests.remove(loadingRequest)
    }
    ...
    private func processPendingRequests() {
        ...
        let requestsFulfilled: Set<AVAssetResourceLoadingRequest> = ResourceLoaderDelegate.pendingRequests.filter {
        ...
        requestsFulfilled.forEach { ResourceLoaderDelegate.pendingRequests.remove($0) }
    }
