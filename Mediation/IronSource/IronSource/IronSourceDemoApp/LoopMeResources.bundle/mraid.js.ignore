...
(function () {
    ...
    mraid.getStringResizeProperties = function () {
        log.i("mraid.getOrientationProperties");
    ...
    mraid.setExpandProperties = function (properties) {
        ...
        log.i("mraid.setExpandProperties " + properties);
    ...
    mraid.getStateString = function () {
        return state;
    }
 
    mraid.sendAnalyticsEvent = function(event, data) {
    ...
    var fireEvent = function (event) {
        ...
        if (listeners[event] && listeners[event].length) {
            for (var i = 0; i < listeners[event].length; i++) {
                var itemName = "i" + i;
                eventListeners[itemName] = listeners[event][i];
            }
        }
        for (var itemName in eventListeners) {
            eventListeners[itemName].apply(null, args);
        }
    };
