...
- (instancetype)initWithAppKey: (NSString *)appKey
    ...
    if (self = [super init]) {
        ...
        _interstitial1 = [LoopMeInterstitialGeneral interstitialWithAppKey: appKey
                                                          preferredAdTypes: adTypes
                                                                  delegate: self
                                                                isRewarded: isRewarded];
        _interstitial2 = [LoopMeInterstitialGeneral interstitialWithAppKey: appKey
                                                          preferredAdTypes: adTypes
                                                                  delegate: self
                                                                isRewarded: isRewarded];
        _delegate = delegate;
...
+ (instancetype)interstitialWithAppKey: (NSString *)appKey delegate: (id<LoopMeInterstitialDelegate>)delegate {
    return [[LoopMeInterstitial alloc] initWithAppKey: appKey
...
+ (instancetype)rewardedWithAppKey: (NSString *)appKey delegate:(id<LoopMeInterstitialDelegate>)delegate {
    return [[LoopMeInterstitial alloc] initWithAppKey: appKey
                                     preferredAdTypes: LoopMeAdTypeAll
                                             delegate: delegate
                                           isRewarded: YES];
...
- (void)loadAdWithTargeting: (LoopMeTargeting *)targeting integrationType: (NSString *)integrationType {
    ...
    [self.interstitial1 loadAdWithTargeting: targeting
                            integrationType: self.integrationType
                                 isRewarded: self.isRewarded];
    if (self.isAutoLoadingEnabled) {
        [self.interstitial2 loadAdWithTargeting: targeting
                                integrationType: self.integrationType
                                     isRewarded: self.isRewarded];
    }
}
...
- (void)loopMeInterstitialDidExpire: (LoopMeInterstitialGeneral *)interstitial {
    if (self.isAutoLoadingEnabled) {
        [interstitial loadAdWithTargeting: self.targeting
                          integrationType: self.integrationType
                               isRewarded: self.isRewarded];
    }
...
- (void)loopMeInterstitial: (LoopMeInterstitialGeneral *)interstitial didFailToLoadAdWithError: (NSError *)error {
    if (self.isAutoLoadingEnabled) {
        ...
        if (self.failCount >= kLoopMeLoadCounter) {
            self.timerToReload = [NSTimer scheduledTimerWithTimeInterval: kLoopMeTimeToReload
                                                                  target: self
