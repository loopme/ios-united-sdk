...
- (void)initAdWithWidth:(int)width height:(int)height viewMode:(NSString *)viewMode desiredBitrate:(double)desiredBitrate creativeData:(NSString *)creativeData {
    ...
    [self invokeVpaidMethod:@"initAd" withArguments:[NSArray arrayWithObjects:@(width), @(height), viewMode, @(desiredBitrate), @{ @"AdParameters" : creativeData }, nil]];
}
...
- (void)invokeVpaidMethod:(NSString *)method withArguments:(NSArray *)arguments {
    ...
    stringParams = [[stringParams stringByReplacingOccurrencesOfString:@"\n" withString:@""] mutableCopy];
    stringParams = [[stringParams stringByReplacingOccurrencesOfString:@"\r" withString:@""] mutableCopy];
