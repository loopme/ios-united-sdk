...
- (BOOL)isViewable:(UIView *)view {
    ...
    if (![self isRectVisible:actualViewFrame inWindow:self.view.window]) {
        return NO;
    }
...
- (BOOL)isRectVisible:(CGRect)rect inWindow:(UIWindow *)window {
    ...
    CGPoint f = CGPointMake((rect.origin.x + rect.size.width) / 2, rect.origin.y);
    CGPoint g = CGPointMake((rect.origin.x + rect.size.width) / 2, rect.origin.y + rect.size.height - 1);
    CGPoint h = CGPointMake(rect.origin.x, (rect.origin.y + rect.size.height) / 2);
    CGPoint i = CGPointMake(rect.origin.x + rect.size.width - 1, (rect.origin.y + rect.size.height) / 2);
    ...
    NSInteger intersectionAreaSum = 0;
    for (UIView *view in hittestViews) {
        if (![self isChildOrEqual:view of:self.view]) {
            intersectionAreaSum += [self intersectionArea:view.frame with:rect];
        }
    }
    
    NSInteger areaRect = rect.size.width * rect.size.height;
    NSInteger hiddenPercentage = intersectionAreaSum * 100 / areaRect;
...
- (BOOL)isChildOrEqual:(UIView *)child of:(UIView *)view {
    return [child isEqual:view] || [child isDescendantOfView:view] || [child isKindOfClass:[UIScrollView class]] || [[child class] isSubclassOfClass:[UIScrollView class]];
}
