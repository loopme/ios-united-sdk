...
@interface LoopMeDiscURLCacheFile : NSObject
...
@implementation LoopMeDiscURLCacheFile


@end

@interface LoopMeDiscURLCache ()
...
@implementation LoopMeDiscURLCache

+ (LoopMeDiscURLCache *)sharedDiscCache {
    static dispatch_once_t once;
    static LoopMeDiscURLCache *sharedDiskCache;
    dispatch_once(&once, ^{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        dispatch_async(self.diskQueue, ^{
            @autoreleasepool {
                ...
                for (LoopMeDiscURLCacheFile *file in expiredFiles) {
                ...
                while ([self sizeOfCacheFilesInArray:cacheFilesSortedByModDate] >= kCacheSoftMaxSize && cacheFilesSortedByModDate.count > 0) {
                    NSString *oldestFilePath = ((LoopMeDiscURLCacheFile *)[cacheFilesSortedByModDate objectAtIndex:0]).filePath;
...
- (NSArray *)expiredCachedFilesInArray:(NSArray *)cachedFiles {
    ...
    for (LoopMeDiscURLCacheFile *file in cachedFiles) {
...
- (NSMutableArray *)cacheFilesSortedByModDate {
    ...
    for (NSString *fileName in sortedFiles) {
        ...
        LoopMeDiscURLCacheFile *cacheFile = [[LoopMeDiscURLCacheFile alloc] init];
...
- (uint64_t)sizeOfCacheFilesInArray:(NSArray *)files {
    ...
    for (LoopMeDiscURLCacheFile *file in files) {
