 ...
 */
@interface OMIDLoopmeAdSessionConfiguration : NSObject
...
/**
 * DEPRECATED.  Create new ad session configuration supplying the owner for both the impression and
 * media events.
 * @param impressionOwner of whether the native or JavaScript layer should be responsible for
 * supplying the impression event.
 * @param videoEventsOwner providing details of who is responsible for triggering video events. This
 * is only required for video ad sessions and should be set to videoEventsOwner:OMIDNoneOwner for
 * display ad sessions.
 * @param isolateVerificationScripts determines whether verification scripts will be placed in a
 * sandboxed environment. This will not have any effect for native sessions.
 * @return A new session configuration instance.  Returns nil and sets error if OM SDK isn't active
...
- (nullable instancetype)initWithImpressionOwner:(OMIDOwner)impressionOwner
                      ...
                      isolateVerificationScripts:(BOOL)isolateVerificationScripts
                                           error:(NSError *_Nullable *_Nullable)error
    __deprecated_msg("Use -[OMIDAdSessionConfiguration "
                     "initWithCreativeType:impressionType:impressionOwner:mediaEventsOwner:"
                     "isolateVerificationScripts:error:]");
